...
level = dc.aggregate(0, {"key":"class","value":1, "operator": "eq"})
f.write("done\n")
coverage = dc.getCoverageView(level["depth"])
...
objects, coverage, level = dc.selectObjectsByCondition(level["depth"],{"key": "compactness", "value": 0.5, "operator": "gt"}, create_level=True)
f.write("done\n")
png.from_array(coverage.getStupidArray().tolist(), 'L').save("result-level-"+str(level["depth"])+".png")
